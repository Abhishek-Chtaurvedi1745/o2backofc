...
const testimonials = [
  ...
  },
  {
    ...
    testimonial:
      'I was drowning in admin tasks, and I just didn’t have the bandwidth to keep up. Hiring a virtual assistant from Outsource2Backoffice was a game-changer. Now I can focus on the things that actually grow my business, while they handle scheduling, emails, and research. Best decision ever.',
    rating: 5,
  },
  {
    image: clineimg4,
  ...
  },
  {
    ...
    testimonial:
      'Their services helped us scale quickly without the overhead of hiring a huge team. From sales support to accounting, everything runs smoothly, and their customer service is top-notch. They’re like an extension of our in-house team.',
    rating: 5,
  },
];
...
const TestimonialCarousel = () => {
  ...
  return (
    <Row className="client-reviews py-md-5 py-4 d-flex justify-content-center align-items-center">
      <Col md={10} xs={11} sm={10}>
        <h1 className="text-center hed1">Client Reviews</h1>
        <p className="text-center hed5 client-reviews-subtitle">
          We don't need to promote our quality, our happy clients do it for us
        </p>
        <Row className="position-relative client-reviews-slider">
          <Swiper
            spaceBetween={20}
            loop={true}
            ...
            className="client-reviews-swiper py-3"
          >
                ...
                <div className="testimonial-card">
                  <div className="profile">
                    <img
                      ...
                      loading="lazy"
                    />
                    <h5 className="mt-3 mb-1 hed3">{item.name}</h5>
                    <p className="text-muted mb-2 hed5">{item.title}</p>
                    <div className="rating mb-2 hed5" aria-label={`${item.rating} out of 5 stars`}>
                      {Array.from({ length: item.rating }, (_, i) => (
                        <span key={i} style={{ color: '#FFD700' }}>★</span>
                      ))}
                    </div>
                    <p className="testimonial-text para">{item.testimonial}</p>
                  </div>
                </div>
              </SwiperSlide>
            ))}
          ...
          <div ref={navigationPrevRef} className="custom-swiper-button-prev" role="button" aria-label="Previous review">
            <IoIosArrowBack />
          </div>
          <div ref={navigationNextRef} className="custom-swiper-button-next" role="button" aria-label="Next review">
            <IoIosArrowForward />
          </div>
        </Row>
      </Col>
